...
class MessagingController(appier.Controller):
    ...
    def send_webhook(self):
        ...
        # extracts the target URLs allowing both the singular "url" and the
        # plural "urls" forms, then normalizes them into a single list
    ...
    def send_web_push(self):
        ...
        # extracts the message and the subscriptions allowing both the plural
        # "subscriptions" list and the singular endpoint/p256dh/auth form, that
        # is normalized into the subscriptions list expected by the messenger
        message = data.get("message")
        ...
        if not subscriptions:
            ...
            subscriptions = [dict(endpoint=endpoint, p256dh=p256dh, auth=auth)]
