...
class Messenger(object):
    ...
    def send_apn(
        self, tokens, message, key_data=None, cer_data=None, sandbox=None, **kwargs
    ):
        """
    ...
    def send_web_push(
        self, subscriptions, message, vapid_private_key=None, vapid_email=None, **kwargs
    ):
        """
...
class _StandaloneAPNHandler(apn.APNHandler):
    ...
    Bypasses the regular handler initialization so that the direct
    sending logic may be reused in the context of the Messenger,
    where there's no server state to resolve subscriptions from.
    """
...
class _NullLogger(object):
    ...
    Used as the fallback logger for the Messenger when no logger is
    provided, so that the handlers may log freely without requiring
    an owner or an external logging infra-structure.
    """
