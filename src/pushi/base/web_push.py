...
class WebPushHandler(handler.Handler):
    ...
    def send(self, app_id, event, json_d, invalid={}):
        ...
        # delegates to the direct send method and verifies the resulting
        # status, logging a warning in case the send operation has failed
        # (eg: missing VAPID credentials) so that the problem is surfaced
        result = self.send_to_subscriptions(
            subscriptions, message, app=app, invalid=invalid
        )
        ...
        **kwargs
        ...
        for sub in subscriptions:
            ...
            try:
                ...
                # or 404 Not Found), removes the subscription from the database, note
                # that the response is explicitly compared against None as an error
                # response (eg: 410) has a "falsy" boolean value under requests
                if (
                    exception.response is not None
                    and exception.response.status_code in (404, 410)
                ):
